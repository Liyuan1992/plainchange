...
3. 我需要担心什么？
4. 我接下来该检查什么？
5. 新增代码从哪里来？*（Git AI 已留下记录时）*

之后，读者可以从一次变化继续理解**这个软件怎么工作**：它的业务流程或能力地图、变化所在步骤、上下游关系，最后才是有来源的技术证据。技术细节始终可查，但不是默认阅读负担。

如果提供了绑定固定版本范围的测试、构建、浏览器、集成或安全收据，报告会把“**已经替你验证**”和“**还有什么没验证、为什么**”分开。剩余项会说明最小验证方法和责任归属，不再把工程命令原样变成软件负责人的作业。

### 通过 Git AI 知道代码从哪里来

当 [Git AI](https://github.com/git-ai-project/git-ai) 已经为所选提交留下来源记录时，PlainChange 会以只读方式读取这个固定版本范围，并在同一份负责人报告中增加“**这次改动从哪里来**”。它会区分 AI、人工和未追踪的新增代码，并显示脱敏后的工具/模型名称与记录到的会话数量。

两者回答的是不同问题：Git AI 记录逐行代码来源；PlainChange 把来源与这次变化、可能影响、证据边界和仍需负责人决定的事项连起来。来源记录永远不会被升级为代码正确、运行成功或可以发布的证明。

Git AI 仍是可选能力。PlainChange 不会自动安装它、添加 hooks、补造缺失历史、打开完整会话，也不会把提示词和对话正文发送给用户配置的模型。按官方说明安装并重启受支持的编码 Agent 后，之后的修改才会留下记录；PlainChange 会自动发现已安装的命令。

## 它怎样工作

```text
固定 Git 改动 + 可选任务上下文 + 可选 Git AI 来源
              ↓
...
完整体验先进行两次语义模型调用：形成有边界的项目理解，再解释这一次固定改动。随后会用受限的翻译调用，在离线报告写入前准备另一种受支持语言。模型只负责提出语言与重要性判断；PlainChange 仍然掌握固定 Git 采集、证据 ID、来源范围、降级规则、不确定性和报告渲染。译文绑定同一报告身份，不能把静态代码升级成真实运行证明。
...
`--human-language` 只接受 `auto`、`en` 或 `zh-CN`，用于选择模型首先生成的语言；`auto` 跟随系统语言，图形引导页跟随浏览器语言。模型增强分析现在会把另一种受支持语言生成为绑定同一身份的报告译文。英文负责人说明中仍有中文时会失败关闭，不会冒充翻译成功。项目原文、原始引用、代码路径、标识符和技术证据仍保留来源语言。
...
离线阅读器只支持英文和简体中文。首次打开会跟随浏览器/系统偏好，手动选择后保存在本机。模型增强分析会在报告离线前写入两套负责人语言：请求语言是原始说明，受限翻译调用生成另一种语言的完整身份绑定投影。缺项、过期身份、空译文或英文投影仍含中文都会失败关闭。引用原话、代码路径、标识符和技术证据保留原文。
...
译文包绑定源报告身份，本地会校验覆盖度和身份；翻译的语义质量仍需人工审阅。手工命令继续用于确定性报告或人工审阅后的替换译文。
...
- `agent-provenance.json`：可选的、经过脱敏的 Git AI 代码来源摘要；不是正确性或运行验证回执。
- `report-translations.json`：绑定报告身份的另一种负责人语言完整投影。
- `report-translation-run-receipt.json`：该投影的脱敏模型、耗时、批次与 Token 元数据。
