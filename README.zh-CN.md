...
如果本机已经安装 [Git AI](https://github.com/git-ai-project/git-ai)，并且所选提交已有来源记录，PlainChange 会以只读方式读取固定版本范围的 JSON 数据，在报告中增加“**这次改动从哪里来**”。这里只展示经过限制的摘要：AI、人工和未追踪的新增代码数量，脱敏后的工具/模型名称，以及记录到的会话数量。

Git AI 是可选能力。PlainChange 不会自动安装它、添加 hooks、打开完整会话，也不会把提示词和对话正文发送给用户配置的模型。以前没有记录的变化不能自动补回。来源记录只能说明代码从哪里来，不能证明功能正确、运行行为或用户影响。

如果决定启用，请按 Git AI 官方说明安装，并重启受支持的编码 Agent，让之后的修改开始留下记录。PlainChange 会自动发现已安装的命令，不需要额外安装 PlainChange 专用 hook。
